...
#define ADD_CUSTOM_SYSCALL
#ifdef ADD_CUSTOM_SYSCALL
static long do_semtimedopk(int semid, struct sembuf *sops,
		unsigned nsops, const struct timespec64 *timeout)
{
	int error = -EINVAL;
	struct sem_array *sma;
...
asmlinkage long sys_sem_p(int semid, int sem_num)
{
	struct sembuf sem;
    sem.sem_num = sem_num;
    sem.sem_op = -1;
    sem.sem_flg = SEM_UNDO;
	return do_semtimedopk(semid, &sem, 1, NULL);
}

asmlinkage long sys_sem_v(int semid, int sem_num)
{
	struct sembuf sem;
    sem.sem_num = sem_num;
    sem.sem_op = 1;
